---
title: "Getting Started with pam"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Getting Started with pam}
...
from species range polygons.
A PAM is a sites-by-species binary table where rows are grid cells and
columns are species; a cell contains `1` when the species range overlaps
that cell and `0` otherwise.
...
1. Load species range polygons as a `terra::SpatVector`.
2. Call `get_geo_pam()` with a desired grid resolution.
...
CRS (decimal degrees for EPSG:4326):
Higher resolution → more grid cells → finer spatial detail.
...
You can pass any `terra::SpatVector` that has a `sciname` column:
each unique value of `sciname` becomes a column in the output PAM.
